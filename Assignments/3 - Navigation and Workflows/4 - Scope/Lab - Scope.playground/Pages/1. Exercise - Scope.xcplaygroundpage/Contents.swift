...
//print("The value of foo is \(foo)")
// cannot find 'foo' in the scope as scope is declared in the for loop block, we can access it inside that block only
...
// x is defined in global score therefore it can be accessed by any block of code below it, x is modified in local scope of for-in loop
// foo is defined and used in local scope of for-in loop
