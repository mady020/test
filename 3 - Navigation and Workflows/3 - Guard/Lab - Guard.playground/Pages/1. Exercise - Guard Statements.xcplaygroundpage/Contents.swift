...
func calculateArea(x: Double, y:Double) -> Double?{
    guard x > 0 , y > 0 else{
        // when the x < 0 and y < 0
        return nil
    }
    return x * y
}
print(calculateArea(x: 1.2, y: 5.0) ?? "Please provide Positive Values!")
print(calculateArea(x: -1.0, y: 5.0) ?? "Please provide Positive Values!")
...
func add(x:Int? , y:Int?) -> Int? {
    guard let a = x ,let b =  y else {
        return nil
    }
    return a + b
}

print(add(x: 1, y: 2) ?? "Please provide non nil values!")
print(add(x: nil, y: 2) ?? "Please provide non nil values!")
/*:
